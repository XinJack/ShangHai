/**
 * 高德地图热力图
 * 基于http://www.patrick-wied.at/static/heatmapjs/index.html 
...
(function() {
    /**
     * 添加热力图的详细坐标点
     * @param {AMap.Map} map 地图对象
     * @param {Obejct} opts 可选项
     * {
     * 		radius: 
     * 		opacity:
     * 		visible:
     ...
     * }
     */
	var Heatmap = AMap.Heatmap = function(map, opts) {
		this._map = map;
		...
		el.style.top = 0;
		el.style.left = 0;
		el.style.width = this._map.getSize().getWidth() + 'px';
		el.style.height = this._map.getSize().getHeight() + 'px';
		this.config.element = el;
		document.getElementsByClassName('amap-overlays')[0].appendChild(el);
		
		this.heatmap = h337.create(this.config);
	}
	
	Heatmap.prototype.pixelTransform = function(p) {
    ...
    Heatmap.prototype.start = function() {
    	var _this = this;
    	AMap.event.addListener(_this._map, 'dragging', function() {
			_this.change();
    	});
    	
    	AMap.event.addListener(this._map, 'zoomchange', function() {
    		_this.change();
    	});
    }
     ...
     * 地图状态改变，内部调用
     */
    Heatmap.prototype.change = function() {
        var curBounds = this._map.getBounds();

        if(curBounds.getNorthEast().equals(this.bounds.getNorthEast()) 
       	  && curBounds.getSouthWest().equals(this.bounds.getSouthWest())) {
        	return;
        }
        this.bounds = curBounds;
        
        var ne = this._map.lnglatToPixel(curBounds.getNorthEast()),
            sw = this._map.lnglatToPixel(curBounds.getSouthWest()),
            topY = ne.y,
            leftX = sw.x,
            h = sw.y - ne.y,
            w = ne.x - sw.x;
            
        this.config.element.style.width = w + 'px';
        this.config.element.style.height = h + 'px';
        this.heatmap.store.get('heatmap').resize();
        
        if (this.lnglats.length > 0) {
            this.heatmap.clear();

            var len = this.lnglats.length;
            d = {
            ...
            while (len--) {
                var lnglat = this.lnglats[len].lnglat;

                if (!curBounds.contains(lnglat)) {
                    continue;
                }

                var divPixel = this._map.lnglatToPixel(lnglat),
                    screenPixel = new AMap.Pixel(divPixel.x - leftX, divPixel.y - topY);
                var roundedPoint = this.pixelTransform(screenPixel);
                d.data.push({
                    x: roundedPoint.x,
                    y: roundedPoint.y,
                    count: this.lnglats[len].c
                });
            }
	 ...
	 * 	{
			...
			data: [
			    //经度、纬度以及权重值
				{lng: 100, lat: 100, count: 80},
				...
			]
		}
	 */
	Heatmap.prototype.setDataSet = function(data) {
		var curBounds = this._map.getBounds();
		var mapdata = {
			max: data.max,
			data: []
		...
		while(dlen--) {
			var lnglat = new AMap.LngLat(d[dlen].lng, d[dlen].lat);
			if(!curBounds.contains(lnglat)) {
				continue;
			}
			this.lnglats.push({
			...
			var divPixel = this._map.lnglatToPixel(lnglat),
				leftX = this._map.lnglatToPixel(curBounds.getSouthWest()).x,
				topY = this._map.lnglatToPixel(curBounds.getNorthEast()).y,
				screenPixel = new AMap.Pixel(divPixel.x - leftX, divPixel.y - topY);
     ...
     */
    Heatmap.prototype.addDataPoint = function(lng, lat, count) {
        var lnglat = new AMap.LngLat(lng, lat),
            point = this.pixelTransform(this._map.lnglatToPixel(lnglat));
    ...
    Heatmap.prototype.remove = function() {
    	this.heatmap.clearup();
    }
    
    AMap.Heatmap = Heatmap;
})();
